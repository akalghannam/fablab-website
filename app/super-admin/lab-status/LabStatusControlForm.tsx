...
export function LabStatusControlForm() {
  ...
  return (
    <div className="glass-card p-6">
      <h2 className="text-white font-bold mb-5">تغيير الحالة</h2>

      {success && (
        <div className="flex items-center gap-3 mb-4 bg-emerald-500/10 border border-emerald-500/20 rounded-lg p-3">
          <CheckCircle size={18} className="text-emerald-400" />
          <p className="text-emerald-400 text-sm font-medium">تم التحديث بنجاح</p>
        </div>
      )}
          ...
          {OPTIONS.map(opt => (
            <button key={opt.value} type="button" onClick={() => setSelected(opt.value)}
              className={`w-full glass-card p-4 flex items-center gap-4 border-2 transition-all text-right ${
                selected === opt.value ? opt.border : 'border-white/10 hover:border-white/20'
              }`}>
              ...
              <div className={`w-5 h-5 rounded-full border-2 flex-shrink-0 flex items-center justify-center ${
                ...
                {selected === opt.value && <div className="w-2.5 h-2.5 rounded-full bg-brand-dark" />}
              </div>
            </button>
          ))}
        </div>

        <Textarea label="ملاحظات (اختياري)" value={notes}
          onChange={e => setNotes(e.target.value)}
          placeholder="أضف تفاصيل..." rows={2} />
